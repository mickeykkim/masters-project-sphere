...
urlpatterns = [
   ...
   # re_path(r'^wearabledata/(?P<pk>[-\w]+)/(?P<pk>\d+)$', views.WearableAnnotationDetailView.as_view(), name='wearableannotation-detail'),
   ...
   # re_path(r'^cameradata/(?P<pk>[-\w]+)/(?P<pk>\d+)$', views.CameraAnnotationDetailView.as_view(), name='cameraannotation-detail'),
]